...
class TestModel(TestCase):
    ...
    def setUp(self):
        response = requests.get(
            'http://api.openweathermap.org/data/2.5/forecast?q=London&units=metric&APPID=457a76f25e6c0e0c0ac1ff4b1f6f0658')
        models.save_to_model(response)
    ...
    def test_get_forecast_from_api_with_current_date(self):
        ...
        result = models.Forecast.get_forecasts(city_name, date)
    ...
    def test_get_frorecast_with_unexists_city(self):
        """Проверка поиска с неточным названием города"""
        city_name = 'Zaporozhye'
        date = datetime.datetime.today().date()
        result = models.Forecast.get_forecasts(city_name, date)
        self.assertEqual(result['forecast_list'][0][0].city.name, 'Zaporizhzhya')
    ...
    def test_forecasts_for_different_citys(self):
        ...
        second_city = 'Tokyo'
        date = datetime.datetime.today().date()
        first_result = models.Forecast.get_forecasts(first_city, date)
        second_result = models.Forecast.get_forecasts(second_city, date)
